function [Err_b,flag]= Soft_Dec_combine(H_css,p,S,max_itr)
   flag = false;
    [R, C_full] = size(H_css);
    C = C_full / 2;
    ...
    llr_v = max_exp(0,gamma)-max_exp(gamma,gamma); %log((3 - 2 * p) / (2 * p));
    ...
    for w = 1:max_itr        
        ...
        for i = 1:R
            ...
            for j = 1:length(neu_chk)
                ...
                % Ensure x does not saturate
               % x = min(max(x, -20), 20);  % Limit range of x for numerical stability
